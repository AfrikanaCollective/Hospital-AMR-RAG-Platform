...
| ARCH-004 | Embedding model config-driven (placeholder, flagged) | `app/config.py`, `app/ingestion/embed.py` | `tests/test_config_model_verification.py` | in progress — Checkpoint 1: `embedding_gateway_url`/`embedding_gateway_api_key` settings scaffolded (config only, no HTTP call) after the operator supplied real self-hosted-gateway details for the `qllama/bge-large-en-v1.5:latest` embedding endpoint (DEVIATIONS.md #42); `embed.py`'s `gateway` branch remains `NotImplementedError` — implementation is Phase 2 |
...
| ARCH-012 | Reranker: config-driven cross-encoder, **decided to run locally** (placeholder model id, flagged) | `app/config.py`, `app/retrieval/rerank.py` | — | in progress — Checkpoint 1: **`RERANKER_BACKEND=local` decided** (not gateway-routed; DEVIATIONS.md #44, supersedes the earlier #43 recommendation). Local-serving design specified (`sentence-transformers.CrossEncoder`, singleton + thread-offload) and `reranker_device`/`reranker_batch_size`/`reranker_max_length` settings + `hf-model-cache` docker volume scaffolded (config/infra only, no inference call) per DEVIATIONS.md #43. `rerank.py`'s `local` branch remains `NotImplementedError` — implementation is Phase 2; `gateway` is not a supported path for this requirement |
