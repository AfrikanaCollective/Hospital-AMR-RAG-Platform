...
| ARCH-034 | Access control (`AuthProvider`, RBAC + RLS + `record_field_policy` + per-agent tool allow-list + purpose-of-use) | `app/auth/**`, `app/api/deps.py`, `app/api/routes/auth.py`, `app/agents/registry.py`, `app/db/session.py` (RLS GUC), `app/db/models/records.py`, `backend/alembic/versions/ba3c23a19ce9_initial_schema.py` | `tests/test_agent_registry.py`, `tests/test_initial_migration.py`, `tests/test_devjwt.py`, `tests/test_auth_routes.py` | in progress — tool allow-list enforced; Phase 2 RLS on `records.patient`/`records.patient_record`/`memory.patient_context` (DEVIATIONS #63); Phase 4: `current_principal` now requires and verifies a real Bearer token (no more permissive dev fallback), `require_role` route guards in place. Still outstanding: `resolve_field_effect`/`record_field_policy` enforcement (`NotImplementedError`), and `get_db`'s RLS GUC wiring from the caller's principal at the specific call sites that hold an authoritative `patient_id` (DEVIATIONS #86). |
