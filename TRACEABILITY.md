...
| PRD-004 | Chunks retain doc id + version + section path + page + char offset | `app/ingestion/chunking.py`, `app/ingestion/chunk_persistence.py`, `app/db/models/corpus.py` | `tests/test_chunking.py`, `tests/test_chunk_persistence.py` | in progress (Phase 2 — `chunk_document` emits the full span per chunk; `persist_chunks` writes `corpus.chunk` rows resolving `parent_ordinal` -> real `parent_chunk_id`, verified against a real Postgres) |
...
| PRD-016 | Citations machine-verifiable against stored chunk text/offsets | `app/citations/model.py` (`verify_citation`, `build_citation`) | `tests/test_citation_model.py`, `tests/test_build_citation.py` | in progress (Phase 1 — verifier implemented; Phase 2 — builder implemented, DEVIATIONS #46) |
...
| PRD-032 | HITL accept axis: full/partial/reject/out-of-scope with defined state effects | `app/hitl/decisions.py` (`apply_decision`, `EFFECTS`), `app/schemas/hitl.py`, `frontend/src/components/AcceptAxisControls.tsx` | `tests/test_hitl_decisions.py`, `tests/test_hitl_routes.py` | **done** (Phase 3 — verified against real Postgres, incl. reject rollback; `out_of_scope` added as a fourth action per operator direction, DEVIATIONS #84 — judges the request, not the answer) |
...
| PRD-041 | Rubric scores stored as structured data (domain/score/rater/ts/result) | `app/db/models/eval.py` (`RubricRating`), `app/rubric/workflow.py` (`submit_rating`) | `tests/test_rubric_workflow.py`, `tests/test_rubric_routes.py` | **done** (Phase 3) |
...
| ID | Requirement (short) | Implementing file(s) | Test(s) | Status |
|---|---|---|---|---|
| PRD-080 | All patient-record fields PHI by default, all environments | `app/schemas/record.py`, `app/db/models/records.py` (`patient.data_class`) | `tests/test_ingest_deidentified.py`, `tests/test_ingest_records_persistence.py` | in progress — **Checkpoint 1 (applied):** `data_class ∈ {synthetic, deidentified}` (DEVIATIONS #33, #37, #38). Phase 2: `payload_enc` envelope encryption + `mrn_hash` lookup-only dedup implemented and verified against a real Postgres (DEVIATIONS #57); RBAC/RLS enforcement is Phase 4. |
...
| PRD-084 | Field-level access control at API + data layer | `app/auth/rbac.py` (`resolve_field_effects`), `app/db/models/records.py` (`RecordFieldPolicy`), `app/records/access.py` (`get_patient_fields`), `app/agents/patient_record_agent.py`, `app/api/routes/records.py` | `tests/test_rbac.py`, `tests/test_records_access.py`, `tests/test_patient_record_agent.py` | in progress — data-layer gate implemented and wired into `get_patient_fields` (fail-closed deny + role/purpose/field wildcard, DEVIATIONS #87); API-layer `GET /records/*` routes still 501 stubs |
...
| PRD-086 | RBAC: clinician, reviewer, admin (+ service) | `app/auth/provider.py`, `app/auth/devjwt.py`, `app/auth/repository.py`, `app/auth/rbac.py`, `app/api/deps.py`, `app/api/routes/auth.py`, `app/db/models/iam.py` | `tests/test_devjwt.py`, `tests/test_auth_repository.py`, `tests/test_auth_routes.py` | in progress (Phase 4 — real dev-JWT issuance + verification, `current_principal` requires a valid token; per-role route guards via `require_role` in place; `resolve_field_effect` field-policy enforcement not started) |
...
| PRD-105 | Redis + Celery for async ingestion + long agent tasks | `app/worker.py`, `app/ingestion/tasks.py`, `app/eval/tasks.py` (`generate_questions`, `run_harness_task`), `app/rubric/tasks.py` (`compute_result_irr_and_archive`, `compute_slice_irr`), `docker-compose.yml` | `tests/test_eval_tasks.py`, `tests/test_rubric_tasks.py` | **done** (Phase 2 ingestion tasks; Phase 3 — eval + rubric task bodies implemented and enqueueing verified against a real Redis broker) |
...
| ARCH-019 | HITL interaction modes (rank axis + accept axis) and their effect on state | `app/hitl/decisions.py` (`EFFECTS`, `apply_decision`), `app/schemas/hitl.py`, `app/schemas/rubric.py`, `app/rubric/workflow.py` | `tests/test_hitl_decisions.py`, `tests/test_rubric_workflow.py` | **done** (Phase 3 — accept axis now 4 actions: full_accept/partial_accept/reject/out_of_scope, DEVIATIONS #84) |
...
| ARCH-034 | Access control (`AuthProvider`, RBAC + RLS + `record_field_policy` + per-agent tool allow-list + purpose-of-use) | `app/auth/**`, `app/api/deps.py`, `app/api/routes/auth.py`, `app/agents/registry.py`, `app/agents/patient_record_agent.py`, `app/records/access.py`, `app/db/session.py` (RLS GUC), `app/db/models/records.py`, `backend/alembic/versions/ba3c23a19ce9_initial_schema.py` | `tests/test_agent_registry.py`, `tests/test_initial_migration.py`, `tests/test_devjwt.py`, `tests/test_auth_routes.py`, `tests/test_rbac.py`, `tests/test_records_access.py`, `tests/test_patient_record_agent.py` | in progress — tool allow-list enforced; Phase 2 RLS on `records.patient`/`records.patient_record`/`memory.patient_context` (DEVIATIONS #63); Phase 4: `current_principal` requires and verifies a real Bearer token (DEVIATIONS #86); `resolve_field_effects`/`record_field_policy` field-level gate implemented (fail-closed deny, exact-match + wildcard, `mask` substitution) and wired into `get_patient_fields`, verified against a real ephemeral Postgres (DEVIATIONS #87). Still outstanding: `get_db`'s RLS GUC wiring from the caller's principal at the specific call sites that hold an authoritative `patient_id`; seeding real default `record_field_policy` rows (`scripts/seed_db.py`). |
| ARCH-035 | Audit logging (append-only grants, hash chain, encrypted text + hashes, one event per action) | `deploy/postgres/init/01_schemas_roles.sql`, `app/db/models/audit.py`, `app/audit/log.py`, `backend/alembic/versions/ba3c23a19ce9_initial_schema.py` | `tests/test_audit_append_only.py`, `tests/test_initial_migration.py` | in progress (Phase 2 — writer implemented for the `retrieval` action, DEVIATIONS #50; append-only now also enforced by a DB trigger — defense in depth beyond the role GRANTs, verified to block UPDATE/DELETE even for the table-owning role (DEVIATIONS #63). `query_text_enc`/`response_text_enc` encryption wiring and the other action types are Phase 3/4) |
