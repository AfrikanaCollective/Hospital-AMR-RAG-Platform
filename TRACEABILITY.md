...
| ID | Requirement (short) | Implementing file(s) | Test(s) | Status |
|---|---|---|---|---|
| PRD-070 | Eval harness: retrieval precision/recall + citation accuracy vs a fixed synthetic set | `app/eval/harness.py` (`run_harness`), `app/eval/metrics.py`, `app/eval/run.py` (CLI) | `tests/test_eval_harness.py`, `tests/test_eval_metrics.py`, `tests/test_eval_run_cli.py` | **in progress** (was **done**, corrected 2026-09-17 — Phase 3 shipped `run_harness`/metrics, but `EvalQuestion.gold_relevant_chunks` was never written by anything upstream, so `_score_one`'s `precision_recall` has been silently `None` for every question in every run to date; citation accuracy is unaffected. `app.eval.auto_seed` now populates it from each scenario's own real citations (DEVIATIONS #122) and 65 pre-existing rows were backfilled the same way — but `app.eval.tasks._run_generate_questions` (the harness's *own* fixed-testset generator) still does not, since it never invokes the real pipeline at generation time; retrieval precision/recall remains structurally `None` for fixed-testset questions specifically until that path is fixed too) |
