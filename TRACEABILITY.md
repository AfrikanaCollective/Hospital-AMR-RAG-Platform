...
| PRD-103 | Embedding + reranker model ids config-driven w/ placeholders | `app/config.py`, `app/ingestion/embed.py`, `app/retrieval/rerank.py` | `tests/test_embed.py`, `tests/test_rerank.py` | **done** (Phase 1 — config surface + stub backends; Phase 2 — `local` backends implemented; post-Phase-5 — `gateway` embedding backend implemented and verified live (real 1024-dim vectors from a real gateway), `EMBEDDING_MODEL_ID=qllama/bge-large-en-v1.5:latest` confirmed against the gateway's own `/metrics`; `RERANKER_BACKEND=local` run against real downloaded weights for the first time this session — DEVIATIONS #103) |
