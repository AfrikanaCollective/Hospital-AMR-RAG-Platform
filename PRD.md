...
| PRD-112 | *(this phase — `UNIFIED-ABLATION-PROPOSAL.md`; DEVIATIONS.md #192-#201; restructured 2026-09-23, operator-supplied hierarchy)* A single hierarchical offline ablation unifying PRD-109/110/111's separate mechanisms into one sweep: **Level 1** (present-only vs. all-assessed clinical-sign query construction), **Level 2** (vocabulary/concept enrichment, reusing PRD-111's mechanism), **Level 3** (a single continuous BM25/SapBERT weighted-rank-fusion sweep, `w_BM25 ∈ {0.0, 0.1, ..., 1.0}` — MedCPT and RRF fusion considered and dropped from scope). **Recall@K is the primary metric** (MRR@K kept secondary); K and the BM25 weight are config-driven, never hardcoded. Per-query results and a reproducibility snapshot (model id/version, K/weight grids, seed, timestamp) persist to disk; comparisons use a **paired** bootstrap CI on the delta (same query indices resampled for both sides), not independent-sample CIs. Evidence-gathering only — no change to the production `/query` path. |
