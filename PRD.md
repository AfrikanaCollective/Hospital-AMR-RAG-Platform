...
| PRD-112 | *(this phase — `UNIFIED-ABLATION-PROPOSAL.md`; DEVIATIONS.md #192/#193)* A single hierarchical offline ablation unifying PRD-109/110/111's separate mechanisms into one sweep: **Level 1** (present-only vs. all-assessed clinical-sign query construction), **Level 2** (vocabulary/concept enrichment, reusing PRD-111's mechanism), **Level 3** (retrieval/embedding configuration — BM25 / BM25+SapBERT / BM25+MedCPT / BM25+SapBERT+MedCPT, each dense-bearing arm alpha-swept). MRR@K is the primary metric; K and alpha are config-driven, never hardcoded. Per-query results and a reproducibility snapshot (model ids/versions, K/alpha grids, seed, timestamp) persist to disk; comparisons use a **paired** bootstrap CI on the delta (same query indices resampled for both arms), not independent-sample CIs. Evidence-gathering only — no change to the production `/query` path. |
