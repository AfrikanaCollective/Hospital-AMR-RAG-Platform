...
audits the exact list read).

**Field-level policy scope (DEVIATIONS.md #70):** `app.auth.rbac.resolve_field_effect`
(the DB-backed `records.record_field_policy` (role, purpose, field_path) ->
allow|deny|mask resolution) is explicitly Phase 4 (its own docstring says so)
— the `iam` roles/policy-seeding work it depends on doesn't exist yet. This
module still enforces data minimization now, just at a coarser, code-level
grain: (a) `get_patient_fields` never returns direct identifiers
(`given_name`/`family_name`/`mrn`) regardless of what is requested — the one
thing every purpose in this system's scope (guideline matching, stage
classification, missing-info) never needs; (b) it returns only the field
paths the caller explicitly asked for, nothing else; (c) every read is
audited with the exact field list, patient_id, and purpose, matching the
audit-completeness requirement even though the allow/deny/mask decision
itself is not yet policy-table-driven. When Phase 4 implements
`resolve_field_effect`, `get_patient_fields` gains a real per-(role, purpose,
field) gate in front of the same projection logic.
...
def get_patient_fields(
    ...
    *,
    ...
    """Return ONLY the requested, authorized field values (ARCH §10.2). Every
    call is audited with the exact field list requested (not what happened to
    exist) — the read itself is the sensitive act, whether or not a field had
    a value."""
    ...
    result = {f: features[f] for f in requested if f in features}
